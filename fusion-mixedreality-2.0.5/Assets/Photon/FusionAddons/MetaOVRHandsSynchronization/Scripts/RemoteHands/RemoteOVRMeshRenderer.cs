...
namespace Fusion.Addons.MetaOVRHandsSynchronization
{
    ...
    /**
     * Replacement of OVRMeshRenderer to connect to RemoteOVRMesh: OVRMesh ShouldInitialize cannot be used for remote hands in the editor, as there is a check on local finger tracking
     ...
     */
    [RequireComponent(typeof(RemoteOVRMesh))]
    public class RemoteOVRMeshRenderer : MonoBehaviour
    {
        ...
        [SerializeField]
        private RemoteOVRMesh _ovrMesh;

        [SerializeField]
        private RemoteOVRSkeleton _ovrSkeleton;
        ...
        private void Awake()
        {
            ...
            if (_ovrMesh == null)
            {
                _ovrMesh = GetComponent<RemoteOVRMesh>();
            }

            if (_ovrSkeleton == null)
            {
                _ovrSkeleton = GetComponent<RemoteOVRSkeleton>();
            }
        }
